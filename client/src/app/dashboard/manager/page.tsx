...
const LandlordPage = () => {
  ...
  const { data: properties, isLoading: propertyLoading } =
    useGetLandlordPropertyQuery(user.user.id || "", { skip: !user?.user?.id });

  if (applicationsLoading || propertyLoading || PaymentLoading || leasesLoading)
    return (
      <div>
        <BouncingLoader />
      </div>
  ...
  return (
    <div className="p-4 *:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4">
      ...
      <Card className="@container/card">
        <CardHeader>
          <CardDescription>Total Properties Leases</CardDescription>
          <CardTitle className="text-2xl font-semibold tabular-nums @[250px]/card:text-3xl">
            {leases?.length || 0}
          </CardTitle>
        </CardHeader>
